...
async function DashboardContent() {
  ...
  const [statsResponse, githubResponse, repoResponse, prsResponse] = await Promise.all([
  ...
  console.log("🚀 ~ DashboardContent ~ prsResponse:", prsResponse)

  const stats = statsResponse.success ? statsResponse.data?.stats : null;
  const githubStatus = githubResponse.success ? githubResponse.data : { connected: false };
  // @ts-ignore - apiService return type structure mismatch
  const repositories = repoResponse.success ? (repoResponse.data?.data || []) : [];
  // @ts-ignore - apiService return type structure mismatch
