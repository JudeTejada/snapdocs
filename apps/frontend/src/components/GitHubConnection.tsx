...
export default function GitHubConnection({ onConnectionChange }: GitHubConnectionProps) {
  ...
  const handleConnectGitHub = async () => {
    try {
      ...
      // 2. Handle the callback with installation ID
      // 3. Call the API to store the connection

      // For demo purposes, we'll simulate the OAuth flow
      const simulatedInstallationId = "123456789"; // This would come from GitHub OAuth
      
      const token = await getToken();
      const response = await apiService.connectGitHub(simulatedInstallationId, token || undefined);
      
      if (response.success) {
        onConnectionChange?.(true, simulatedInstallationId);
      } else {
        setError(response.error || "Failed to connect GitHub");
      }
    } catch (error) {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: '1rem' }}>
        ...
        <div>
          ...
          <p style={{ color: '#666', fontSize: '0.9rem' }}>
            Connect your GitHub repositories to start auto-generating documentation
          </p>
        </div>
      </div>
      ...
      <div style={{ marginBottom: '1rem' }}>
        ...
        <ul style={{ marginTop: '0.5rem', paddingLeft: '1.5rem', fontSize: '0.9rem', color: '#555' }}>
      ...
      <p style={{ 
        ...
        GitHub App OAuth required. This will redirect to GitHub for authorization.
